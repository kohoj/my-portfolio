<template>
	<div
		...
		:id="{ works: !i }"
		:class="[
			{ 'w-full h-full carousel-item': isPhone },
			'sm:border-2 sm:rounded-box',
		]"
	...
	>
		...
		>
					...
					<div class="transform -translate-y-10 rotate-12">
						<div class="animation-10s justify-start">
							<div
								...
								:key="j"
								v-show="
									(item.photos.length > 6 && item.photos.length < 11) ||
									(item.photos.length <= 6 ? j < 3 : j < 8)
								"
								class="mr-8 h-56 rounded-box bg-cover bg-center shadow-lg"
								:style="{
									width: item.photos.length > 6 ? '7rem' : '22rem',
			...
			<!-- words -->
			<div
				class="card-body max-h-80 relative z-20 items-center justify-start overflow-hidden"
			>
				<div class="rounded-box bg-neutral text-left px-3 flex relative">
					<div class="max-w-sm flex flex-col relative z-20">
						...
						<!-- detail -->
						<div v-for="(p, i) in item.detail.split('||')" :key="i">
							<p class="text-sm sm:text-base mb-3">{{ p }}</p>
						</div>
						<!-- stacks -->
